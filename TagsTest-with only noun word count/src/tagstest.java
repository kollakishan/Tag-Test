		 ...
		 int counter = 0;
	        ...
	        for (int i = 0; i < b.length; i++)
	        {
	            ...
	            freqMap.put(key, ++freq);
	            	...
	            	{
	            		String c[] = result.getKey().split("_");
	            		for(int j=0; j < c.length; j++)
	        	    	{
	            			if (n_word.equals(c[j]))
	       	    		 	{
	            				System.out.println( c[j-1]+ " " + result.getValue());
	            				//result.getKey()
	       	    		 	}
	            			if (np_word.equals(c[j]))
	       	    		 	{
	            				System.out.println( c[j-1]+ " " + result.getValue());
	            				//result.getKey()
	       	    		 	}
	            			if (ns_word.equals(c[j]))
	       	    		 	{
	            				System.out.println( c[j-1]+ " " + result.getValue());
	            				//result.getKey()
	       	    		 	}
	            			if (nps_word.equals(c[j]))
	       	    		 	{
	            				System.out.println( c[j-1]+ " " + result.getValue());
	            				//result.getKey()
	       	    		 	}
